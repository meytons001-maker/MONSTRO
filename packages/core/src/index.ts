...
export * from "./mission-capability-requirements.ts";
...
export class MonstroOrchestrator {
  ...
  private async runToDelivery(task: MonstroTask, plan: BuildPlan, trace: MissionTraceCollector): Promise<Delivery> {
    ...
    while (task.iteration < task.maxIterations) {
      ...
      await this.journal.record(task, "runtime.completed", runtime.ok ? `runtime completed in ${runtime.durationMs}ms` : `runtime failed in ${runtime.durationMs}ms`, {
        ok: runtime.ok,
        durationMs: runtime.durationMs,
        previewUrl: runtime.previewUrl,
        stdoutLength: runtime.stdout.length,
        stderrLength: runtime.stderr.length,
      });
