...
function normalizeRequirementIds(value: unknown, allowed: ReadonlySet<string>, path: string): string[] {
  if (!Array.isArray(value) || value.length === 0) throw new Error(`AI Repairer requirementIds are required for ${path}`);
  const ids = value.map((id) => { if (typeof id !== "string" || !allowed.has(id)) throw new Error(`AI Repairer requirementId is invalid for ${path}: ${String(id)}`); return id; });
  return [...new Set(ids)];
}
function normalizePatch(value: RawPatch, allowedPaths: ReadonlySet<string>, allowedRequirementIds: ReadonlySet<string>): FilePatch {
  ...
  if (value.operation !== "create" && value.operation !== "update" && value.operation !== "delete") throw new Error(`AI Repairer operation is invalid for ${path}`); if (value.operation === "delete") return { path, operation: "delete", requirementIds };
  if (typeof value.content !== "string") throw new Error(`AI Repairer content is required for ${path}`); if (Buffer.byteLength(value.content, "utf8") > MAX_CONTENT_BYTES) throw new Error(`AI Repairer content is too large for ${path}`); return { path, operation: value.operation, content: value.content, requirementIds };
}
export async function generateAiRepair(router: ModelRouter, task: MonstroTask, evaluation: Evaluation, artifacts: AiRepairArtifact[]): Promise<AiRepairResult | undefined> {
  ...
  if (tracedIds.size === 0) return undefined;
  const response = await router.generate({
    capability: "code",
    system: "You are the MONSTRO Repairer. Return only JSON. Repair only explicitly authorized target files inside the isolated workspace. Satisfy the traced build requirements behind each repair action. Every patch must declare one or more requirementIds from the traced requirements. Never request credentials, bypass authentication/DRM, modify .git, escape the workspace, or propose unauthorized access.",
    prompt: JSON.stringify({ intent: task.intent, understanding: task.context.understanding, requirements, findings: evaluation.findings, actions: evaluation.nextActions, artifacts: selectedArtifacts, allowedPaths: [...allowedPaths], responseSchema: { patches: [{ path: "one of allowedPaths", operation: "create|update|delete", content: "required except delete", requirementIds: ["one or more traced requirement IDs"] }] } }),
  ...
  const patches = raw.map((patch) => normalizePatch(patch, allowedPaths, tracedIds)); if (new Set(patches.map((patch) => patch.path)).size !== patches.length) throw new Error("AI Repairer returned duplicate patch paths"); return { provider: response.provider, model: response.model, patches };
}
