...
import { PublicUrlInspector, extractPublicHttpUrl, inventoryClientAssets } from "./index.ts";
...
test("produces bounded document, network and asset evidence", async () => {
  const inspector = new PublicUrlInspector({ fetchImpl: async () => new Response("<!doctype html><title>Reference</title><h1>World</h1><script src='/app.js'></script><img src='x'><source src='/scene.glb'>", { status: 200, headers: { "content-type": "text/html" } }) });
