...
function view(state: MissionExecutionState, phase?: MissionPipelinePhase): MissionConsoleView {
  return { operational: { state, phase, label: [state.toUpperCase(), phase?.toUpperCase()].filter(Boolean).join(" · ") }, pipeline: [], preview: { available: false, label: "WAITING FOR BUILD" }, resume: { available: false, label: "READ ONLY" }, evidence: [], feed: [] };
}
