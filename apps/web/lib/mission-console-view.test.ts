...
test("derives operational state, pipeline, preview and history resume state", () => {
  ...
  assert.equal(Object.hasOwn(view, "activePhase"), false);
  assert.equal(Object.hasOwn(view, "executionState"), false);
...
test("derives completed and failed terminal operational states", () => {
  ...
  assert.deepEqual(completed.operational, { state: "completed", phase: "deliver", label: "COMPLETED · DELIVER" });
  ...
  assert.deepEqual(failed.operational, { state: "failed", phase: undefined, label: "FAILED" });
...
test("effective detail and matching server snapshot override stale history", () => {
  ...
  assert.equal(view.resumeLabel, "REBUILD FROM INSPECT"); assert.equal(view.resumeReason, "workspace missing"); assert.equal(view.canResume, true); assert.equal(view.previewUrl, "/preview/restored"); assert.equal(view.operational.phase, "build");
});
...
test("falls back to replaying live events after a restored snapshot becomes stale", () => {
  ...
  assert.equal(view.operational.phase, "run"); assert.equal(view.previewUrl, "/preview/live");
});
...
test("unselected cockpit is idle and read only", () => {
  ...
  assert.deepEqual(view.operational, { state: "idle", phase: undefined, label: "IDLE" });
