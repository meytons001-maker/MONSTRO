...
import { createModelRouterFromEnvironment, generateAiBuild, generateAiPlan, generateAiRepair, requirementsFromUnderstanding } from "@monstro/ai";
...
export function createV0Services(task: MonstroTask): MonstroServices {
  ...
  return {
    ...
    architect: { async plan(current, evidence) { const understanding = current.context.understanding; const requirements = requirementsFromUnderstanding(understanding); try { const aiPlan = await generateAiPlan(ai, current.intent, evidence.map((item) => `${item.source}: ${item.summary}`), understanding); if (aiPlan) { current.context.decisions.push(`AI Architect planned ${requirements.length} requirement(s) with ${aiPlan.provider}/${aiPlan.model}`); return { taskId: current.id, rationale: aiPlan.rationale, requirements, steps: aiPlan.steps.map((step, index) => ({ id: `ai-${index + 1}`, title: step.title, description: step.description, status: "pending" as const })) }; } } catch (error) { current.context.decisions.push(`AI Architect unavailable; deterministic fallback used: ${error instanceof Error ? error.message : "unknown error"}`); } return { taskId: current.id, rationale: understanding ? `${understanding.rationale}; deterministic plan derived from ${evidence.length} evidence item(s)` : `Deterministic plan derived from ${evidence.length} evidence item(s)`, requirements, steps: [{ id: "v0-web", title: understanding?.interactivity === "interactive" ? "Build interactive web preview" : "Build reachable web preview", description: current.intent, status: "pending" }] }; } },
